...
public final class PageModel {
    ...
    public List<Map<String,Object>> products(Long storeId,boolean activeOnly)throws SQLException{
        return rows(c,"SELECT p.*,cat.name category,COALESCE(sp.price,p.price) effective_price,(SELECT MIN(i.id) FROM product_images i WHERE i.product_id=p.id) image_id FROM products p JOIN categories cat ON cat.id=p.category_id LEFT JOIN store_prices sp ON sp.product_id=p.id AND sp.store_id=?"+(activeOnly?" WHERE p.active=1":"")+" ORDER BY p.id DESC",storeId);
    }
