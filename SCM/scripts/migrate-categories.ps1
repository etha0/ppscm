...
if (!(Test-Path -LiteralPath (Join-Path $projectRoot 'target\classes\kr\bujobank\scm\CategoryMigration.class'))) { throw 'Build the project with mvn package before migrating.' }
& "$JavaHome\bin\java.exe" -cp $classPath kr.bujobank.scm.CategoryMigration
if ($LASTEXITCODE -ne 0) { throw 'Category migration failed. Keep the application stopped and inspect the database before retrying.' }